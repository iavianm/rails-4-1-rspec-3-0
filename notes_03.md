...
Для начала спецификация модели должна включать следующие тесты:
>- Создание модели при допустимых атрибутах должно быть допустимым.
>- Данные, не прошедшие проверку, не должны быть действительными.
>- Методы класса и экземпляра работают должным образом.
...
```
Это простая спецификация простой модели, но она описывает четыре лучших практики:
- Описывается набор ожиданий - как должна выглядеть модель Contact и как себя вести. 
- Каждый запрос, только для одной проверки. 
>Тестируются имя, фамилия и электронная почта отдельно. Таким образом, если тест терпит неудачу, то сразу понятно в чем ошибка

- Каждый запрос является явным.
- Описание каждого запроса начинается с глагола not should.
* * *
## **Создание model spec**
...
```
>В этом примере используется средство сопоставления `be_valid`, чтобы убедиться, что наша модель выглядит нужным образом
...
Рассмотрим некоторые детали нашей spec'и для проверки имени:
...
Мы ожидаем, что новый контакт (с именем, явно установленным nil )
...
Проверка адреса электронной почты, что адрес должен быть уникальным, также достаточно проста:
...
и так далее, но не Джонса. Есть несколько способов реализовать это - для
