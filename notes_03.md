...
Проверяем это с помощью `include` matcher'а, который проверяет значение сообщения об ошибке. `can't be blank`
...
>Тесты нужны для того, что бы не упустить мелкие детали (например валидации) в процессе разработки.
>Если исходить из принципа TDD (`Test-Driven-Development`), то вероятность, пропустить какой либо функционал сводиться к минимуму.
